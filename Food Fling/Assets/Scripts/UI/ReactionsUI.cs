...
{
    public GameObject reactionGameObject; // The game object containing the TextMeshProUGUI component
    ...
    private void Start()
    {
        ...
        reactionText = reactionGameObject.GetComponentInChildren<TextMeshProUGUI>();
        ...
        reactionGameObject.gameObject.SetActive(false);
    }
    ...
    {
        if (reactionGameObject == null || reactionText == null) return;
        ...
        reactionGameObject.GetComponent<RectTransform>().anchoredPosition = uiPos;

        // Get reaction text based on the reward value
        reactionText.text = GetReactionText(orderData.AccuracyScore);

        // Show the reaction
        reactionGameObject.gameObject.SetActive(true);
